...
class TasksBloc extends HydratedBloc<TasksEvent, TasksState> {
  TasksBloc() : super(TasksState()) {
    ...
    on<PermenentlyDeleteTask>(_onPermentlyDeleteTask);
  }
  ...
  void _onUpdateTask(UpdateTask event, Emitter<TasksState> emit) {
    ...
    List<Task> completedList = state.completedTasks;
    List<Task> favoriteList = state.favoriteTasks;
    if (oldTask.isDone == false) {
      if (oldTask.isFavorite == false) {
        pendingList = List.from(pendingList)..remove(oldTask);
        completedList = List.from(completedList)
          ..insert(0, oldTask.copyWith(isDone: true));
      } else {
        var taskIndex = favoriteList.indexOf(oldTask);
        pendingList = List.from(pendingList)..remove(oldTask);
        completedList = List.from(completedList)
          ..insert(0, oldTask.copyWith(isDone: true));
        favoriteList = List.from(favoriteList)
          ..remove(oldTask)
          ..insert(taskIndex, oldTask.copyWith(isDone: true));
      }
    } else {
      if (oldTask.isFavorite == false) {
        completedList = List.from(completedList)..remove(oldTask);
        pendingList = List.from(pendingList)
          ..insert(0, oldTask.copyWith(isDone: false));
      } else {
        var taskIndex = favoriteList.indexOf(oldTask);
        completedList = List.from(completedList)..remove(oldTask);
        pendingList = List.from(pendingList)
        ...
        favoriteList = List.from(favoriteList)
          ..remove(oldTask)
          ..insert(taskIndex, oldTask.copyWith(isDone: false));
      }
    }
    emit(TasksState(
  ...
  void _onPermentlyDeleteTask(
      PermenentlyDeleteTask event, Emitter<TasksState> emit) {
    final state = this.state;
    emit(TasksState(
